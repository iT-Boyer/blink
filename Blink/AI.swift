...
fileprivate class AIOverlayController: UIImagePickerController {
...
fileprivate final class AIView: UIView, UIGestureRecognizerDelegate {
  private let _ctrl = AIOverlayController()
...
class AIManager {
  private var _view: AIView?
  private var _spaceCtrl: SpaceController? = nil
  private var _state: AIView.State = .init(frame: .zero, flipped: false)
  
  init() {
    _view = AIView(frame: CGRect(origin: .zero, size: CGSize(width: 80, height: 80)))
  ...
  private static var __shared: AIManager? = nil
  ...
  @objc func moveToForeground(notificationInfo: NSNotification) {
    DispatchQueue.main.asyncAfter(deadline: .now() + 1.0 ) {
      ...
      let view = AIView(frame: CGRect(origin: .zero, size: CGSize(width: 80, height: 80)))
      self._view = view
